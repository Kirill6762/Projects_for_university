# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 20:18:10 2024
...
class Blockchain:   
    ...
    def postgre (): 
        conn = psycopg2.connect(database='postgres', user='postgres', password='Krecker13', host='localhost', port='5432')
        cursor = conn.cursor() 
        cursor.execute('SELECT * from "Galbert"')
        df = cursor.fetchall() 
        df = pd.DataFrame(df) 
        print (df) 
        return df 
...
# Эта функция ниже создана для добавления дополнительных блоков в цепочку 
    def create_block(self, proof, previous_hash): 
        ...
        block = {'index': len(self.chain) + 1, 
                  ...
                  'company_name': str(df[2].iloc[len(self.chain)]),
                  'place': str(df[3].iloc[len(self.chain)]),
                  'cash_flow': str(df[4].iloc[len(self.chain)]),
                  'foundation_date': str(df[4].iloc[len(self.chain)]),
                  'proof': proof, 
